...
radiologia.shift(radiologia.pop());

//3.Lista consultas dentales

document.write(`<p><strong>CONSULTAS DENTALES</strong></p>`);
dental.forEach(function(dental) {
    document.write(`<p>${dental.hora} - ${dental.especialista} - ${dental.paciente} -${dental.rut} - ${dental.prevision}</p>`);
    });

    //4. Imprimir un listado total de todos los pacientes que se atendieron en el centro médico. 
document.write(`<p><strong>LISTADO DE TODOS LOS PACIENTES</strong></p>`);
var pacientes = radiologia.concat(traumatologia.concat(dental));
pacientes.forEach(function(paciente){
    document.write(`<p>${paciente.paciente}</p>`)
});

    //5. Modificar mediante funciones las previsiones de Dental
    dental = dental.map(function(cambiar) {
        if (cambiar.prevision === "FONASA") {
        cambiar.prevision = "ISAPRE";
        } else {
        cambiar.prevision = "FONASA";
        }
        return cambiar;
        });

        dental.forEach(function(consuDental){
            document.write(`<p>${consuDental.prevision}; ${consuDental.paciente}</p>`)
        })
